...
// ─── FIX: update-activity — replaced broken router.handle() internal re-routing
//          with direct inline logic for each activityType.
//          The old router.handle() approach silently failed in Express, meaning
//          stream/assignment/assessment counts were NEVER incremented via this route.
router.post('/update-activity', async (req, res) => {
  try {
    ...
    // ── Handle each activityType inline (no internal re-routing) ──────────────
    if (activityType === 'streams') {
      const meetingId = itemData.id || itemData.meetingId || new mongoose.Types.ObjectId();
      ...
      if (!exists) {
        activity.activities.streams.items.push({
          id: meetingId,
          title: itemData.title || itemData.meetingTitle || 'Untitled Meeting',
          type: itemData.type || itemData.meetingType || 'live_stream',
          scheduledTime: itemData.scheduledTime || now,
      ...
      return res.status(400).json({
        success: false,
        error: `Invalid activityType: "${activityType}". Must be one of: streams, assignments, assessments`
      });
    }
...
router.get('/staff/:staffId/timeline', async (req, res) => {
  try {
    ...
    // REAL meetings count from Meeting collection
    const meetings = await Meeting.find({
      ...
      .select('_id title classId createdAt')
      .lean();

    const uniqueMeetingMap = new Map();
    meetings.forEach((meeting) => {
      uniqueMeetingMap.set(String(meeting._id), meeting);
    });
    const uniqueMeetings = Array.from(uniqueMeetingMap.values());
    ...
    // Add stream timeline from real meetings
    uniqueMeetings.forEach((meeting) => {
      timeline.push({
        type: 'stream',
        title: meeting.title || 'Untitled Stream',
...
const uniqueClasses = new Set(docs.map((d) => String(d.classId)).filter(Boolean));

// Count REAL meetings by classId
const meetingCountByClassId = {};
uniqueMeetings.forEach((meeting) => {
  const classKey = meeting.classId ? String(meeting.classId) : '__no_class__';
  meetingCountByClassId[classKey] = (meetingCountByClassId[classKey] || 0) + 1;
});

// Merge class-wise activity from StaffActivity docs
const classBreakdownMap = new Map();

docs.forEach((doc) => {
  const classKey = doc.classId
    ? String(doc.classId)
    : `no_class_${doc.className || 'unknown'}`;

  if (!classBreakdownMap.has(classKey)) {
    classBreakdownMap.set(classKey, {
      classId: doc.classId ? String(doc.classId) : null,
      className: doc.className || 'Unknown Class',
      subject: doc.subject || doc.classSubject || 'N/A',
      section: doc.section || doc.classSection || 'N/A',
      streams: 0,
      assignments: 0,
      assessments: 0
    });
  ...
  const existing = classBreakdownMap.get(classKey);

  existing.assignments += Number(
    doc.totalAssignments || doc.activities?.assignments?.count || 0
  );

  existing.assessments += Number(
    doc.totalAssessments || doc.activities?.assessments?.count || 0
  );
});

// Inject REAL stream counts into each class row
for (const [classKey, item] of classBreakdownMap.entries()) {
  item.streams = meetingCountByClassId[classKey] || 0;
}

// If a class has meetings but no StaffActivity doc yet, still include it
Object.entries(meetingCountByClassId).forEach(([classKey, count]) => {
  if (!classBreakdownMap.has(classKey) && classKey !== '__no_class__') {
    classBreakdownMap.set(classKey, {
      classId: classKey,
...
const classBreakdown = Array.from(classBreakdownMap.values());
    ...
    res.status(200).json({
      ...
      summary: {
        ...
        totalStreams: uniqueMeetings.length,
