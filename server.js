...
try {
  if (process.env.FIREBASE_SERVICE_ACCOUNT) {
    ...
    console.log('Firebase: Loaded from FIREBASE_SERVICE_ACCOUNT (JSON)');
  } else {
    serviceAccount = {
    ...
    if (!serviceAccount.projectId || !serviceAccount.clientEmail || !serviceAccount.privateKey) {
      throw new Error('Missing Firebase credentials (projectId, clientEmail, privateKey)');
    }
    console.log('Firebase: Loaded from separate env variables');
  }

  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
    storageBucket: 'uelms-378db.appspot.com',
  });
  ...
  if (process.env.FIREBASE_SERVICE_ACCOUNT) {
    console.error('FIREBASE_SERVICE_ACCOUNT starts:', process.env.FIREBASE_SERVICE_ACCOUNT.substring(0, 150) + '...');
  }
  console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  process.exit(1);
}
...
app.use('/api/google-meet', googleMeetAttendanceRoutes);
...
app.get('/api/status', (req, res) => {
  const apiStatus = {
    google: { enabled: !!(process.env.GOOGLE_API_KEY && process.env.GOOGLE_CX) },
    serpapi: { enabled: !!process.env.SERPAPI_KEY },
    core: { enabled: !!process.env.CORE_API_KEY },
    crossref: { enabled: !!process.env.CROSSREF_EMAIL }
  };

  res.json({
    success: true,
    message: 'API Status',
    server: { status: 'running', port: PORT, environment: process.env.NODE_ENV || 'development' },
    apis: apiStatus,
...
app.get('/api/staff-with-passwords', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch staff with passwords: ' + err.message });
  }
});
...
app.get('/api/students-with-passwords', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch students with passwords: ' + err.message });
  }
});
...
app.get('/api/staff/:identifier/classes', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch staff classes: ' + err.message });
  }
});
...
app.get('/api/staff/email/:email/classes', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch classes by email: ' + err.message });
  }
});
...
app.get('/api/classes', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch classes: ' + err.message });
  }
});
...
app.post('/api/staff-with-password', async (req, res) => {
  ...
  try {
    ...
    if (existingStaff) return res.status(400).json({ success: false, error: 'Staff email already exists in database' });
      ...
      if (firebaseErr.code === 'auth/user-not-found') {
        try {
          ...
          if (createErr.code === 'auth/email-already-exists') {
            return res.status(400).json({ success: false, error: 'Email already exists in Firebase' });
          }
          throw createErr;
        }
      } else {
    ...
    if (err.code === 11000) return res.status(400).json({ success: false, error: 'Staff email already exists in database' });
    if (err.code === 'auth/email-already-exists') return res.status(400).json({ success: false, error: 'Email already registered in Firebase' });
    if (err.name === 'ValidationError') {
      const messages = Object.values(err.errors).map(val => val.message);
      return res.status(400).json({ success: false, error: 'Validation error: ' + messages.join(', ') });
    }

    res.status(500).json({ success: false, error: 'Failed to add staff: ' + err.message });
  }
});
...
app.post('/api/students-with-password', async (req, res) => {
  ...
  try {
    ...
    if (existingStudent) return res.status(400).json({ success: false, error: 'Student email already exists in database' });
    ...
    if (err.code === 11000) return res.status(400).json({ success: false, error: 'Student email already exists in database' });
    if (err.code === 'auth/email-already-exists') return res.status(400).json({ success: false, error: 'Email already registered in Firebase' });
    if (err.name === 'ValidationError') {
      const messages = Object.values(err.errors).map(val => val.message);
      return res.status(400).json({ success: false, error: 'Validation error: ' + messages.join(', ') });
    ...
    res.status(500).json({ success: false, error: 'Failed to add student: ' + err.message });
  }
});
...
app.post('/api/bulk-users-enhanced', async (req, res) => {
  ...
  try {
    ...
    console.log('Enhanced bulk upload:', { type, userCount: users?.length, sampleUser: users?.[0] });

    if (!type || !['staff', 'student'].includes(type)) {
      return res.status(400).json({ success: false, error: 'Invalid or missing type (staff|student).' });
    }
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      if (!email || typeof email !== 'string') {
        results.push({ row: rowNumber, email: email || 'unknown', name: name || 'unknown', success: false, error: 'Email is required' });
        continue;
      }

      if (!password || typeof password !== 'string') {
        results.push({ row: rowNumber, email: email, name: name || 'unknown', success: false, error: 'Password is required' });
        continue;
      }

      if (!name || !name.trim()) {
        results.push({ row: rowNumber, email: email, name: name || 'unknown', success: false, error: 'Name is required' });
        continue;
      }

      const cleanProgram = (program || '').trim();
      if (type === 'staff' && !cleanProgram) {
        results.push({ row: rowNumber, email: email, name: name, success: false, error: 'Department/Program is required' });
        continue;
      }

      if (password.length < 6) {
        results.push({ row: rowNumber, email: email, name: name, success: false, error: 'Password must be at least 6 characters' });
        continue;
      }

      if (!emailRegex.test(email)) {
        results.push({ row: rowNumber, email: email, name: name, success: false, error: 'Invalid email format' });
        continue;
      }
      ...
      try {
        try {
          ...
          if (err.code === 'auth/user-not-found') {
            try {
              ...
              results.push({ row: rowNumber, email: lowerEmail, name: name, success: false, error: 'Firebase creation failed: ' + createErr.message });
              continue;
            }
          } else {
            throw err;
        ...
        if (type === 'staff') {
          ...
          if (existingStaff) {
            ...
            if (!existingStaff.passwordHistory) existingStaff.passwordHistory = [];
            ...
            results.push({ row: rowNumber, email: lowerEmail, name: name, success: true, action: 'updated' });
          } else {
            ...
            results.push({ row: rowNumber, email: lowerEmail, name: name, success: true, action: 'created' });
          }
        } else {
          const existingStudent = await Student.findOne({ email: lowerEmail });
          if (existingStudent) {
        ...
        results.push({ row: rowNumber, email: lowerEmail, name: name, success: false, error: err.message });
    ...
    res.status(500).json({ success: false, error: 'Failed to process bulk upload: ' + err.message });
  }
});
...
app.post('/api/bulk-users-with-passwords', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to bulk create users: ' + err.message });
  }
});
...
app.post('/api/clear-temp-passwords', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to clear temporary passwords: ' + err.message });
  }
});
...
app.put('/api/users', async (req, res) => {
  try {
    ...
    if (!oldEmail || !type) {
      return res.status(400).json({ success: false, error: 'Old email and type are required' });
    }
    ...
    if (newEmail && newEmail !== oldEmail) {
      ...
      if (existing && existing._id.toString() !== user._id.toString()) {
        return res.status(400).json({ success: false, error: 'New email already exists' });
      }
